...
export class RLNBoard {
  ...
  public async addMember(account: Account, secret: bigint): Promise<IMTNode> {
    ...
    if (leaves.includes(identityCommitment)) {
      console.warn(`Member with identity commitment ${identityCommitment} already exists. Skipping addition.`);
      return this.tree.root;
    }
    const mintToken = await this.program.mintToken(account.toString(), 100);
    const mintTokenRecord = mintToken[0];
    this.tokenRecords.set(account.toString(), RecordCiphertext.fromString(mintTokenRecord));

    const tokenRecord = this.tokenRecords.get(account.toString()); 
    ...
    this.tokenRecords.set(account.toString(), RecordCiphertext.fromString(newTokenRecord));
  ...
  public async postMessage(account: Account, secret: bigint, message: string): Promise<void> {
    ...
    const messageCounterRecord = this.userMessageCountRecords.get(account.toString());
    ...
    this.userMessageCountRecords.set(account.toString(), RecordCiphertext.fromString(newMessageCounterRecord));
    ...
    if (exceedFlag === "false") {
      this.messages.set(account.toString(), message);
    }
  }
  ...
  public async slash(slasher: Account, slasherSecret: bigint, slasheeIdentityCommitment: bigint, xShare0: bigint, xShare1: bigint, yShare0: bigint, yShare1: bigint): Promise<void> {
    const identityCommitment = poseidon2([slasherSecret]);  
    const index = this.tree.indexOf(identityCommitment);
    const proof = this.tree.createProof(index);
    const tokenRecord = this.tokenRecords.get(slasher.toString()); 
    ...
    const newTokenRecord = outputs[0];
    this.tokenRecords.set(slasher.toString(), RecordCiphertext.fromString(newTokenRecord));
  }
}
