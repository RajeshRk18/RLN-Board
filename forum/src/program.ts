import { ProgramManager, AleoKeyProvider, NetworkRecordProvider, AleoNetworkClient, initThreadPool } from '@provablehq/sdk';
...
export class RlnProgram {
     ...
     */
    async executeTransition(functionName, inputs, keySearchParams, fee = DEFAULT_FEE, privateFee = false) {
    ...
    async mintToken(receiver, amount) {
     ...
     */
    async register(groupId, secret, token, root, old_root) {
        const inputs = [
            `${groupId}field`,
            `${secret}field`,
            `${token.owner}address`,
     ...
     * @param root - The new Merkle tree root.
     */
    async postMessage(groupId, grpNullifier, address, secretKey, index, message, msgCounterRecord, merklePathElements, merklePathIndices, root) {
     ...
     * @param root - The new Merkle tree root.
     */
    async withdrawStake(groupId, member, secret, index, token, msgCounterRecord, merklePathElements, merklePathIndices, root) {
        const inputs = [
            `${groupId}field`,
            `${member}address`,
            `${secret}field`,
     ...
     * @param root - The new Merkle tree root.
     */
    async slash(groupId, slasher, slasherSecret, slasheeIdentityCommitment, xShare0, xShare1, yShare0, yShare1, token, merklePathElements, merklePathIndices, root) {
        const inputs = [
            `${groupId}field`,
            `${slasher}address`,
            `${slasherSecret}field`,
    ...
    async getGroupNullifier(groupId) {
